...
function adminAuth(req, res, next) {
  ...
  if (pass !== process.env.ADMIN_PASSWORD)
    return res.status(401).json({ ok: false, error: 'Admin password salah' });
  next();
}
...
router.post('/create-user', adminAuth, async (req, res) => {
  try {
    ...
    res.json({ ok: true, message: `User ${username} (${plan}) berhasil dibuat` });
  } catch (e) {
...
router.patch('/set-plan', adminAuth, async (req, res) => {
  try {
    ...
    res.json({ ok: true, message: `${username} diupgrade ke ${plan.toUpperCase()}` });
  } catch (e) {
    res.json({ ok: false, error: e.message });
  }
});
